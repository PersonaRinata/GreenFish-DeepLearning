...
class TextRNN(object):
    ...
    def rnn(self):
        ...
        def lstm_cell():  # lstm核
            return tf.compat.v1.nn.rnn_cell.BasicLSTMCell(self.config.hidden_dim, state_is_tuple=True)

        def gru_cell():  # gru核
            return tf.compat.v1.nn.rnn_cell.GRUCell(self.config.hidden_dim)
        ...
        def dropout():  # 为每一个rnn核后面加一个dropout层
            if (self.config.rnn == 'lstm'):
                ...
                cell = gru_cell()
            return tf.compat.v1.nn.rnn_cell.DropoutWrapper(cell, output_keep_prob=self.keep_prob)
        ...
        with tf.name_scope("rnn"):
            ...
            _outputs, _ = tf.compat.v1.nn.dynamic_rnn(cell=rnn_cell, inputs=embedding_inputs, dtype=tf.float32)
            last = _outputs[:, -1, :]  # 取最后一个时序输出作为结果

        with tf.name_scope("score"):
            # 全连接层，后面接dropout以及relu激活
            fc = tf.compat.v1.layers.dense(last, self.config.hidden_dim, name='fc1')
